...
const AddItemModalContent: FC<AddItemModalContentProps> = ({ onClose }) => {
  ...
  const disabled = !price || !selectedTypeOfOrder || !selectedCustomer || !selectedRestaurant
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setSelectedRestaurant(restaurantResponse.data[0]);
        ...
        setSelectedTypeOfOrder(typeOfOrderResponse.data[0])
  ...
  const createCustomerOrder = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:3001/customer-order', body);
